...
export default function CommentBubbleElement({
  ...
  const toggleMinimize = useCallback((e: React.MouseEvent) => {
    ...
    const currentSizeNumeric = {
      width: typeof currentSize.width === 'number' ? currentSize.width : parseFloat(String(currentSize.width)) || 240,
      height: typeof currentSize.height === 'number' ? currentSize.height : parseFloat(String(currentSize.height)) || 140,
    };

    if (isCurrentlyMinimized) {
      const { originalSize, ...restProps } = (properties || {}) as Partial<CanvasElementProperties>;
      const restoredSize = originalSize || { width: 240, height: 140 };
      ...
      onUpdate(id, {
        ...
        content: commentContent, // Asegurar que el contenido se preserve
      });
      onSelectElement(id, false);
    } else {
      // Guardar el contenido actual antes de minimizar
      const updatedContent = { ...commentContent, text: text };
      const currentWidth = typeof currentSize.width === 'number' ? currentSize.width : parseFloat(String(currentSize.width)) || 240;
      onUpdate(id, {
        minimized: true,
        properties: {
          ...safeProps,
          originalSize: currentSizeNumeric,
          size: { width: currentWidth, height: 52 },
        },
        content: updatedContent, // Guardar el contenido actualizado
      });
    }
  }, [id, minimized, onSelectElement, onUpdate, safeProps, commentContent, text, isPreview]);
